...
def process_chunk_worker(core_id, queue):
    while True:
        try:
            main_logger.info(f"Core {core_id} attempting to get chunk from the queue...")
            chunk_data, chunk_name = queue.get(timeout=1)  # Timeout helps periodically check for new chunks
            main_logger.info(f"Core {core_id} got chunk: {chunk_name} from the queue")
            if chunk_data is None:  # None indicates no more chunks to process
                break
            ...
            # Example callback function - replace this with your actual callback logic
            callback_function(result, core_id)
        except Q.Empty:
            # Queue is empty, no more chunks to process
            main_logger.info(f"Core {core_id} - Queue is empty. No more chunks to process.")
            break
        except Exception as e:
            main_logger.error(f"Core {core_id} - Exception occurred: {e}")
...
def process_records():
    ...
    for chunk, chunk_name in zip(records_chunks, chunk_names):
        queue.put((chunk, chunk_name))

    # Print the length of the queue before starting the workers
    main_logger.info(f"Queue length before starting workers: {queue.qsize()}")

    # Create and start process workers
    processes = []
    for core_id in range(num_cores):
        main_logger.info(f"\n\nAssigning Chunks to Core ID: {core_id}.....\n\n")
        process = multiprocessing.Process(target=process_chunk_worker, args=(core_id, queue))
