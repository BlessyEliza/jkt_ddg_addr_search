...
# Semaphore to limit concurrent tasks for getAddress function
semaphore_getAddress = asyncio.Semaphore(20)  # Adjust the number as per your requirement

# Semaphore to limit concurrent tasks for get_public_ip function
semaphore_get_public_ip = asyncio.Semaphore(10)  # Adjust the number as per your requirement
...
async def aget_results(word):
    async with semaphore_getAddress:
...
async def getAddress(phrase):
    async with semaphore_get_public_ip:
        logging.debug(f"Getting address for phrase: {phrase}")
        return await asyncio.gather(*[aget_results(phrase) for _ in range(5)])
...
def main():
    ...
    with open(OUTPUT_CSV_FILE_PATH, mode, newline='', encoding='utf-8') as file:
        ...
        for index, row in tqdm(df.iloc[start_index:].iterrows(), initial=start_index, total=df.shape[0]):
            ...
            # Initialize CSV writer after the first row is ready to determine headers
            if writer is None:
                writer = csv.DictWriter(file, fieldnames=row.index.tolist())
                if mode == 'w':  # Write the header only for a new file
                    writer.writeheader()

            writer.writerow(row.to_dict())
            save_checkpoint(index)
            api_requests_counter.inc()  # Increment the Prometheus counter for each API request
